...
func TestBucket_OpenReader_TrackNotFound(t *testing.T) {
	_, err := (&Bucket{Store: memstore.New()}).OpenReader(t.Context(), testTrack)
...
func TestBucket_OpenReader_RejectsManifestForAnotherTrack(t *testing.T) {
	...
	_, err := (&Bucket{Store: objects}).CreateTrack(t.Context(), "live/cam1/video", testConfig(t))
	require.NoError(t, err)
	...
	_, err = (&Bucket{Store: objects}).OpenReader(t.Context(), "live/cam2/video")
